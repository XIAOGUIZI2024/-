...
# 偏向锁升级为轻量锁的过程
### 一、批量重偏向（Bulk Rebiasing）

- **触发条件**：一个偏向锁对象被多个线程争抢，当争抢次数达到 **20 次**（默认阈值）后触发。
    
- **发生过程**：
    
    - 前20次偏向锁仍偏向原线程。
        
    - 当第 **18 次** 争抢（或触发阈值附近）时，JVM会开始将后续争抢的对象**重新偏向**给新的线程（如线程B）。
        
    - JVM认为这个对象“更适合”新线程，因此将锁偏向给新线程，而不是继续撤销升级。
        
- **目的**：减少锁撤销开销，适应多线程轻度竞争场景。
    

---

### 二、批量撤销（Bulk Revocation）

- **触发条件**：
    
    - 在批量重偏向的基础上，如果锁争抢次数达到 **40 次**（默认阈值）。
...
- **发生过程**：
    ...
    - 之后该类所有新创建的对象，**直接以轻量级锁开始**。
        
- **意义**：JVM认为该类竞争激烈，偏向锁已无优势，因此彻底放弃偏向锁机制。
...
- 真正的锁升级是**基于Class（类元数据）** 的，而不是基于单个对象实例。
    
- 当某个类触发批量撤销后，该类后续所有实例都将跳过偏向锁，直接从轻量级锁开始竞争。
    
- 这一机制是JVM对锁竞争模式的自适应优化，旨在减少不必要的偏向锁开销。
# 轻量级锁升级为重量级锁
- 锁升级（从轻量级锁到重量级锁）是针对**具体对象实例**的，即每个 `new` 出来的对象都有自己的锁状态。
    
- 触发升级的条件是：**线程竞争激烈**，且 **CAS（Compare And Swap）操作失败**。
    
- 此时会触发 **锁膨胀** 过程，将轻量级锁升级为重量级锁，重量级锁会依赖操作系统层面的互斥锁机制，性能开销更大。
# 从轻量级锁升级到重量级锁**

#### **触发条件**

- 当线程在轻量级锁状态下竞争失败（如自旋超过阈值、或有多线程竞争），锁会升级为重量级锁。
#### **升级步骤（由持有锁的线程1执行）**

1. **初始化Monitor对象**
    
    - 线程1创建ObjectMonitor对象，用于重量级锁管理。
        
2. **设置状态为“膨胀中”（inflating）**
    
    - 标记锁正在升级，防止其他线程干扰。
        
3. **复制Mark Word到Monitor**
    
    - 将轻量级锁中Lock Record里的Mark Word（含hashcode、分代年龄、偏向锁标志）复制到Monitor的header属性中。
        
4. **修改对象头指向重量级锁**
    
    - 将对象头的标记位改为`00`（重量级锁状态）。
        
    - 前30位指向刚初始化的Monitor对象。
...
6. **线程2竞争重量级锁**
    ...
    - 线程2尝试获取重量级锁，但因为是**非公平锁**，不一定能立即获取到。
        
    - 线程2在升级过程中起到了“奠基者”作用，触发了锁升级，但自身不一定能获得锁。
# **避免死锁的常见方法**

#### 1. **避免单线程持有多个锁**
- 一个线程应尽量避免同时获取多个锁，减少锁的依赖链，降低死锁概率。
#### 2. **避免在锁内占用多个资源**
- 每个锁应尽量只管理一个资源，资源分离有助于减少竞争和死锁的可能性。
#### 3. **使用定时锁**
- 使用 `lock.tryLock(timeout)` 替代内置锁机制，设置获取锁的超时时间，避免无限期等待。
#### 4. **数据库锁要在同一连接中**
- 数据库的加锁与解锁操作必须在同一个数据库连接中执行，否则可能导致解锁失败或锁未释放。