...
System.out.println(t.getState());  // 只代表检查瞬间的状态
```
...
// ✅ 正确做法：使用线程间通信机制
// 使用 wait/notify、CountDownLatch、CyclicBarrier 等
```
# **常见问题**

### Q1: 线程状态会跳过某个状态吗？

**A:** 会。例如，线程可以直接从 NEW → TERMINATED（如果立即调用 `run()` 而不是 `start()`）。

### Q2: `isAlive()` 和 `getState()` 有什么区别？

```java

Thread t = new Thread();
t.isAlive();     // false - 线程是否启动过
t.getState();    // NEW - 当前状态
...
| 状态                | 描述    | 触发条件                       | 退出条件                      |
| ----------------- | ----- | -------------------------- | ------------------------- |
| **NEW**           | 新建未启动 | `new Thread()`             | 调用 `start()`              |
| **RUNNABLE**      | 可运行   | `start()`                  | 执行结束或进入等待                 |
| **BLOCKED**       | 阻塞等待锁 | 等待 synchronized            | 获取到锁                      |
| **WAITING**       | 无限期等待 | `wait()`, `join()`         | `notify()`, `notifyAll()` |
| **TIMED_WAITING** | 有限期等待 | `sleep()`, `wait(timeout)` | 超时或被唤醒                    |
| **TERMINATED**    | 终止    | `run()` 结束                 | 无                         |
